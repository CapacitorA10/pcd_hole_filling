...
colorInfo = np.ones([len(mesh.vertices),3])/1.5 #미리 데이터의 컬러를 설정\
...
for i in range(len(mesh.triangles)) :
    ...
    if np.squeeze(number_of_tri) == 1 :
        ...
        for j in range(3) :
            colorInfo[need_to_red[0,j]] = [1,0,0]
    ...
    '''2번 꼭짓점'''
    temp = np.where(np.asarray(mesh.triangles) == a_triangular[1])[0]  # 첫 번째 꼭짓점에 맞닿은 index
    second = np.asarray(mesh.triangles)[temp]

    """2번 to 3번"""
    number_of_tri = np.shape(np.where(second == a_triangular[2])[0])  # 첫 번째 꼭짓점에 연결된 삼각형들을 조사하여, 두 번째 꼭짓점에 맞닿는 애들을 조사
    # 이 때, 2라면, 2개의 삼각형이므로 면, 1이라면 1개의 삼각형이므로 hole
    if np.squeeze(number_of_tri) == 1:
